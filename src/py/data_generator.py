...
class Data_generator():
    ...
    def get_plot_function(self):
        def plot_function(level=0.5, hold=1, participation=1.0):
            """
            函数应该根据return_vectors生成三个平均收益率和方差的双列序列，其中，参数应该包括投资者参数，超参数
            投资者参数应该包括：投资水平，持有时长，投资参与度
            超参数应该包括：投资者数量，交易成本
            """
            result = [(0.0, 0.0)] * len(self.return_vectors)
            num_holds = -(-len(self.return_vectors[0]) // hold)
            adjusted_returns = [[math.prod(vector[j * hold: min((j + 1) * hold, num_holds)]) for j in range(num_holds)] for (i, vector) in enumerate(self.return_vectors)]
