    ...
    def plot(self):
        ...
        for i, level in enumerate([0.55, 0.5, 0.45]):
            for j, participation in enumerate([1.0, 0.8, 0.6, 0.4, 0.2]):
                ...
                # Index
                y = [data[0][0] for data in data_list]
                y_lower: list[float] = [data[0][0] - data[0][1] * shift for data in data_list]
                y_upper: list[float] = [data[0][0] + data[0][1] * shift for data in data_list]
                ax[i][j].plot(x, y)
        ...
        fig.savefig(self.save_path + "/index.png", dpi=300)
